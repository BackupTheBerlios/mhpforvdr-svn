...
public void addNotify () {
   //only lightweight components in MHP, don't do anything
...
public Graphics getGraphics () {
   if ( (flags & IS_ADD_NOTIFIED) != 0 )
      return MHPScreen.createClippedGraphics(this);
   else
      return null;
}
...
Component getToplevel () {
   Component c;
   for ( c=this; !(c instanceof MHPPlane) && c != null; c= c.parent );
   return c;
}

public MHPPlane getMHPPlane() {
   Component top=getToplevel();
   if (top instanceof MHPPlane)
      return (MHPPlane)top;
   else 
      return null;
}
...
void processPaintEvent ( int id, int ux, int uy, int uw, int uh ) {
   //MHP: do nothing here - let HScene do it.
   System.out.println("Component.processPaintEvent called!! Backtrace:");
   (new Exception()).printStackTrace();
   /*MHPNativeGraphics g = (MHPNativeGraphics)getGraphics();
...
//Bring it up to MHPPlane, reapint is overridden there
public void repaint ( long ms, int x, int y, int width, int height ) {
   if ( (flags & IS_SHOWING) == IS_SHOWING ){
      ...
      if (parent != null)
         parent.repaint(x+getX(), y+getY(), width, height);
...
public void transferFocus() {
   ...
   while (curr.parent != null) {
      ...
      Container par = curr.parent;
      int end = par.getComponentCount();
      ...
      for (start = 0; start < end; start++) {
         Component c = par.getComponent(start);
         if (c == curr) {
            break;
         }
      }
