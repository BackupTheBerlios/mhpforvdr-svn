...
public class EventQueue {
    static Category CAT = Category.getInstance( EventQueue.class );
    AWTEvent localQueue;
    AWTEvent localEnd;
    ...
    synchronized void dropPaintEvents
        ( Object source, int x, int y, int w, int h ) {
            AWTEvent e = localQueue;
            AWTEvent del, last = null;
            while ( e != null ) {
                if ( ( e.id == PaintEvent.UPDATE ) &&
                    e.isObsoletePaint( source, x, y, w, h ) ) {
                        if ( localEnd == e ) {
                            localEnd = last;
                        }
                        if ( last == null ) {
    ...
    public AWTEvent getNextEvent() {
		...
		// returned 'null'
                if ( !AWTEvent.accelHint ) {
                    ...
                    if ( Defaults.EventPollingRate < 10 ) {
                        Defaults.EventPollingRate += 3;
                    }
                }
            }
        }
    ...
    public synchronized void postPaintEvent
        ( int id, Component c, int x, int y, int width, int height ) {
            AWTEvent e = localQueue;
    //    CAT.debug("posting paint event for "+ c );
    //if( c == null ) {
    //	Thread.dumpStack();
    //}
    ...
    // scan the localQueue twice (it might get large)
            if ( e != null ) {
                do {
                    if ( ( e.id == id ) &&
                        ( ( PaintEvt )e ).solicitRepaint( c, x, y, width,
                        height ) ) {
                            return;
                    }
                    if ( e.next == null ) {
                ...
                e.next = localEnd =
                    PaintEvt.getEvent( c, id, 0, x, y, width, height );
            } else {
                localQueue = localEnd =
                    PaintEvt.getEvent( c, id, 0, x, y, width, height );
                if ( ( Toolkit.flags &
                    Toolkit.NATIVE_DISPATCHER_LOOP ) != 0 ) {
                        notify(); // wake up any waiter
