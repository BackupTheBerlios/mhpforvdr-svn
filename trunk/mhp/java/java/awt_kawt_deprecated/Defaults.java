 ...
 * when referenced inside of this class. Copyright (c) 1998
...
class Defaults {
    /**
     * If ConsoleClass is set to the classname of a kaffe.util.log.LogClient,
     * System.out, err are redirected to a kaffe.util.log.LogStream, which
     * notifies the provided LogClient (usually a simple Window displaying
     * scrolled text). If ConsoleClass is null, no io redirection is performed
     */
    ...
    /**
     * maximum x,y coordinate distance (in pixels) between successive
     * MOUSE_PRESSED events which are still considered to be a click (required
     * for some devices without exact mouse positioning).
     */
    static int ClickDistance = 0;

    /**
     * Sleep time (in ms) between native getNextEvent calls (= event polling
     * rate). Just used in case the native layer does not support blocked IO.
     */
    static int EventPollingRate = 10;

    /**
     * Delay (in ms) between background native flush calls (from the flush
     * thread) in case the native layer needs flushing (queued graphics
     * systems like X). This is only required for apps doing graphics output
     * outside the event dispatch thread, and for
     * native layers doing blocked IO (in this case the background drawing
     * wouldn't appear until something is going on in the dispatcher thread)
     */
    static int GraFlushRate = 250;

    /**
     * Physical screen size in 1/10 inches (used for scaling purposes,
     * e.g. fr printing)
     */
    static int ScreenSize = 130;

    /**
     * just a shadow from the Defaults screen width (computed by
     * the native layer)
     */
    static int ScreenWidth;

    /**
     * just a shadow from the Defaults screen height (computed by
     * the native layer)
     */
    ...
    /**
     * Frame decoration extends (titlebar, size-borders) in pixels. For native
     * windowing systems (like X) this is just a guess, to be used for the
     * first Frame to be opened, and later on corrected (Frame.frameDeco). For
     * standalone envionments, this is an order
     */
    static Insets FrameInsets = new Insets( 0, 0, 0, 0 );

    /* (24,4,4,4); */

    /**
     * Dialog decoration extends (titlebar, size-borders) in pixels. For
     * native windowing systems (like X) this is just a guess, to be used for
     * the first Dialog to be opened,
     * and later on corrected (Dialog.dialogDeco). For standalone envionments,
     * this is an order
     */
    static Insets DialogInsets = new Insets( 0, 0, 0, 0 );

    /* (24, 4, 4, 4); */

    /**
     * Native font description string for Default font. This can be used to
     * customize fonts
     */
    static String FsDefault;

    /**
     * Native font description string for Monospaced font. This can be used to
     * customize fonts
     */
    static String FsMonospaced;

    /**
     * Native font description string for SansSerif font. This can be used to
     * customize fonts
     */
    static String FsSansSerif;

    /**
     * Native font description string for Serif font. This can be used to
     * customize fonts
     */
    static String FsSerif;

    /**
     * Native font description string for Dialog font. This can be used to
     * customize fonts
     */
    static String FsDialog;

    /**
     * Native font description string for DialogInput font. This can be used
     * to customize fonts
     */
    static String FsDialogInput;

    /**
     * Native font description string for ZapfDingbats font. This can be used
     * to customize fonts
     */
     ...
     * be matched exactly. It is only used in PseudoColor visuals (256 color
     * modes), where we have to approximate Java 8-8-8 RGB colors by
     * scarce colorcells.
     */
     ...
     */
    static Color FocusClr = Color.red;
    ...
    /**
     * If set to 'true', menu text will be drawn "carved" (with a
     * down-right highlight)
     */
    ...
    /**
     * If set to 'true', non-focused TextAreas keep their cursor displayed (in
     * inactive color)
     */
    ...
    /**
     * If set to 'true', Label text will be drawn carved (with
     * right-down highlight)
     */
     ...
     */
    static Color BtnTxtClr = Color.blue;
    ...
    /**
     * If set to 'true', Button text will be drawn carved (with
     * right-down highlight)
     */
    ...
    /**
     * Image filename to be used to tile RootWindow background (only for
     * standalone environments)
     */
    static String RootWindowImg = "marble.jpg";

    /**
     * Name of KeyTable translation class (only used in
     * standalone environments)
     */
    ...
    /**
     * Color for active window titlebar background (only used in
     * standalone environments)
     */
    ...
    /**
     * Color for inactive window titlebar background (only used in
     * standalone environments)
     */
    ...
    /**
     * Name of class to be used to display and control minimized windows (only
     * for standalone environments)
     */
    static String MinBoxClass = "java.awt.WinTaskBar";

    /**
     * Maximum size of image files to be produced immediately (when the
     * corrseponding image is created). Images with files exceeding this size
     * will be produced asynchronously on demand
     */
    static int FileImageThreshold = Integer.MAX_VALUE;

    /**
     * Maximum size of image data buffers to be produced immediately (when the
     * corrseponding image is created). Images with data buffers exceeding
     * this size will be produced asynchronously on demand
     */
    static int DataImageThreshold = Integer.MAX_VALUE;

    /**
     * Maximum size of MemoryImageSource data to be produced immediately (when
     * the corrseponding image is created). Images with data buffers exceeding
     * this size will be produced asynchronously on demand (currently,
     * everything != 0 will cause async production)
     */
	...
	// we assume a 4:3 width/height ratio of the display
        XResolution = ( int )( ( double )ScreenWidth * 10 /
            ( 0.8 * ScreenSize ) ); // dpi
        YResolution = ( int )( ( double )ScreenHeight * 10 /
            ( 0.6 * ScreenSize ) ); // dpi
        ...
        } else if ( ( version.startsWith( "ggi" ) ) ||
            ( version.startsWith( "DirectFB" ) ) ) {
	        // We set at fist the mapping to the Windows fonts.
	        // Be careful! (dirkh@prz.tu-berlin.de)
	        // This is only usefull in combination with Freetype-Lib
                FsDefault = "arial%s%s.ttf"; //use a font which suits better to a TV screen
                FsMonospaced = "cour%s%s.ttf";
                FsSansSerif = "arial%s%s.ttf";
                FsSerif = "times%s%s.ttf";
                FsDialog = "cour%s%s.ttf";
                FsDialogInput = "cour%s%s.ttf";
                FsZapfDingbats = "wingding.ttf";
	...
	// Display startup banner, if any
        if ( ( banner = System.getProperty( "awt.banner",
            "banner.gif" ) ) != null ) {
