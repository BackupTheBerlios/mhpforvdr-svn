...
// make sure the EventDispatch gets into the top-level thread group
    EventDispatchThread( ThreadGroup tg, EventQueue queue ) {
        super( tg, "AWT-EventQueue-0" ); // some apps depend on this JDK thread name
        this.queue = queue;
                ...
                try {
                    while ( !stop ) {
						...
						// save postEvents AND make dispatching faster
                            if ( ( modalWindow.flags &
                                Component.IS_ADD_NOTIFIED ) == 0 ) {
                                    return;
                            }
                        }
                    }
