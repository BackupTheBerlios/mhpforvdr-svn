...
/**
 * Frame - Copyright (c) 1998 Transvirtual Technologies, Inc.  All rights
...
public class Frame extends Window {
	...
	// setFrameInsets() from the native layer)
        ...
        frameInsets = new Insets( guess.top, guess.left, guess.bottom,
            guess.right );
        menuFrameInsets = new Insets( guess.top + Defaults.MenuBarHeight,
            guess.left, guess.bottom, guess.right );
        frameDeco = new Rectangle( guess.left, guess.top, ( guess.left + guess.right ),
            ( guess.top + guess.bottom ) );
        Toolkit.wndSetFrameInsets( guess.top, guess.left, guess.bottom,
            guess.right );
    }
	...
	// has to compute the real deco offsets during window creation
        ...
        nativeData = Toolkit.wndCreateFrame( title, u, v, w, h, cursor.type,
            bgClr.nativeValue, bgAlpha, ( ( flags & IS_RESIZABLE ) != 0 ) );
    }
	...
	// DEP - should be in doLayout()
        super.layout();
        if ( bMenu != null ) {
            bMenu.setBounds( frameDeco.x, frameDeco.y, width - ( frameDeco.width ),
                Defaults.MenuBarHeight );
        }
    }
...
//synchronized ( treeLock ) {
        ...
        if ( ( ( flags & IS_IN_UPDATE ) == 0 ) && ( bMenu != null ) ) {
            ...
            n = 1;
            g.clipRect( deco.x, insets.top, width - ( deco.width ),
                height - ( deco.height ) );
        }
...
//}
    ...
    public void setResizable( boolean isResizable ) {
        ...
        if ( nativeData != 0 ) {
            Toolkit.wndSetResizable( nativeData, isResizable, x, y,
                width, height );
        }
    }
