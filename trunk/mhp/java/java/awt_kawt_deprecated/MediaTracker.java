...
/**
 * MediaTracker - Copyright (c) 1998 Transvirtual Technologies, Inc.  All
 * rights reserved. See the file "license.terms" for information on usage and
...
public class MediaTracker implements Serializable {
    ...
    final static int LOADED = ImageObserver.ALLBITS | ImageObserver.FRAMEBITS |
        ImageObserver.ABORT;
    ...
    public synchronized boolean checkAll( boolean load ) { //TODO: Fix this again before release!
        for ( MediaTrackerEntry e = images; e != null; e = e.next ) {
            if ( ( e.img.checkImage( e.w, e.h, e, load ) &
                ( ImageObserver.ALLBITS | ImageObserver.FRAMEBITS ) ) == 0 ) {
                    return ( true ); // (false); I'm going to burn in hell for that. joe@convergence.de
            }
        }
        return ( true );
    }
    ...
    public synchronized boolean checkID( int id, boolean load ) {
        for ( MediaTrackerEntry e = getNextEntry( id, null ); e != null;
            e = getNextEntry( id, e ) ) {
                if ( ( e.img.checkImage( e.w, e.h, e, load ) &
	...
	// expensive than creating temp. object arrays
        for ( e = images; e != null; e = e.next ) {
            if ( ( e.img.checkImage( e.w, e.h, e, false ) &
	...
	// with getErrorsAny()
        for ( e = images; e != null; e = e.next ) {
            if ( ( e.id == id ) && ( e.img.checkImage( e.w, e.h, e, false ) &
                ( ImageObserver.ERROR | ImageObserver.ABORT ) ) != 0 ) {
                    n++;
            }
        }
        if ( n > 0 ) {
            ...
            for ( e = images, i = 0; ( i < n ); e = e.next ) {
                if ( ( e.id == id ) &&
                    ( e.img.checkImage( e.w, e.h, e, false ) &
                    ( ImageObserver.ERROR | ImageObserver.ABORT ) ) != 0 ) {
                        a[i++] = e.img;
                }
            }
    ...
    public synchronized void removeImage( Image image, int id, int w, int h ) {
        ...
        MediaTrackerEntry curr = images;
        while ( curr != null ) {
            if ( curr.img == image && curr.id == id && curr.w == w &&
                curr.h == h ) {
                    if ( prev != null ) {
                        prev.next = curr.next;
                    } else {
    ...
    public synchronized void waitForAll() throws InterruptedException {
        System.out.println( "MediaTracker.waitForAll() called. images = " +
            images + ", going to redirect you to " + "waitForAll(500 ms);" );
        waitForAll( 500 );
        System.out.println( "MediaTracker.waitForAll() done." );

        /* for ( MediaTrackerEntry e = images; e != null; e = e.next ) {
        if ((e.img.checkImage( e.w, e.h, e, true) & LOADED) == 0) {
        synchronized ( e ) { e.wait(); } } } */
    }
...
class MediaTrackerEntry implements ImageObserver {
    Image img;
    int id;
    int w;
    int h;
