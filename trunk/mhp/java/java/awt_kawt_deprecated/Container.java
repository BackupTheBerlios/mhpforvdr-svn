...
abstract public class Container extends Component {
    ...
    protected void addImpl( Component child, Object constraints, int index ) {
		...
		// inherit parent attributes (if not overriden by child)
            if ( ( flags & ( IS_PARENT_SHOWING | IS_VISIBLE ) ) ==
                ( IS_PARENT_SHOWING | IS_VISIBLE ) ) {
		...
		// (wouldn't it be nice to have a single LayoutManager interface?)
            if ( layoutm != null ) {
                if ( layoutm instanceof LayoutManager2 ) {
                    ( ( LayoutManager2 )layoutm ).addLayoutComponent( child,
                        constraints );
                }
    ...
    Graphics clipSiblings( Component child, DVBNativeGraphics g ) {
        ...
        for ( i--; i >= 0; i-- ) {
            ...
            cxw = c.x + c.width;
            cyh = c.y + c.height;
            if ( ( c.x > xwClip ) || ( c.y > yhClip ) || ( cxw < xClip ) ||
                ( cyh < yClip ) ) {
                    continue;
            }
		// not yet implemented
    ...
    // This looks too similiar to DVBNativeGraphics.paintChild(), but we can't
    ...
    // (and that is NOT specified!!)
	  ...
	  // Also shows up in DVBNativeGraphics.paintChild()
                    Toolkit.eventQueue.postPaintEvent(
                        ( ( c.flags & IS_ASYNC_UPDATED ) != 0 ?
                        PaintEvent.PAINT : PaintEvent.UPDATE ),
                        c, uxx, uyy, uww, uhh );
                }
    ...
    public Component locate( int x, int y ) {
        Component c;
        if ( !isShowing() || ( x < 0 ) || ( x > this.x + width ) ||
            ( y < 0 ) || ( y > this.y + height ) ) {
                return null;
        }
        for ( int i = 0; i < nChildren; i++ ) {
                    ...
                    ( c.y < uyh ) ) {
                        ...
                        if ( c instanceof Container ) {
                            ...
                            int uww = ( uxw < cxw ) ?
                                ( uxw - c.x - uxx ) : c.width;
                            int uhh = ( uyh < cyh ) ?
                                ( uyh - c.y - uyy ) : c.height;
                            ( ( Container )c ).markRepaints( uxx, uyy,
                                uww, uhh );
                        }
                }
            }
    ...
    void processPaintEvent( int id, int ux, int uy, int uw, int uh ) {
        DVBNativeGraphics g = DVBNativeGraphics.getClippedGraphics( null, this,
            0, 0, ux, uy, uw, uh, false );
        if ( g != null ) {
    ...
    public void update( Graphics g ) {
        if ( isShowing() ) {
            // flags |= IS_IN_UPDATE;
            if ( ( flags & IS_NATIVE_LIKE ) != 0 ) {
                /*            System.out.println("java.awt.Container clearing
                background..."); Thread.dumpStack();*/
        ...
        /* flags |= IS_IN_UPDATE; // clear and draw yourself
        g.clearRect( 0, 0, width, height); paint( g); flags &= ~IS_IN_UPDATE;*/
    }

    public void validate() {
        synchronized( treeLock ) {
            if ( ( flags & ( IS_VALID | IS_ADD_NOTIFIED ) ) ==
                IS_ADD_NOTIFIED ) {
    ...
    public void paintComponents( Graphics g ) {
        /* hotfix joe@convergence.de, 25.Juni 2001 */
	//paintAll( g );
        for ( int i = nChildren - 1; i >= 0; i-- ) {
            Component c = children[i];
            if ( ( c.flags & IS_VISIBLE ) != 0 ) {
