 ...
 * traversal). The answer is WeakReference. Our ref graph looks like this:
 * (per-Graphics cache) +---------------------+   ...
 * |                     |    ^ |          (weak)     V    |       native-like
 * DVBNativeGraphics  <- - - - -  GraphicsLink <-- Component
 * |                                    ^
 * |                                    |
 * +------------------------------------+
 * The trick is to have no strong references from the Component
 * to the Graphics objects. If somebody forgets about the Graphics.dispose,
 * the link will be removed the next time the list is traversed (either when
...
public class GraphicsLink extends WeakReference {
    ...
    GraphicsLink( DVBNativeGraphics g ) {
        super( g );
    }

    boolean updateGraphics( Component tgt ) {
        DVBNativeGraphics g = ( DVBNativeGraphics )get();
        if ( g != null ) {
            boolean isTgtVisible = ( ( tgt.flags & tgt.IS_SHOWING ) ==
                tgt.IS_SHOWING );
            if ( isTgtVisible != isVisible ) {
                /* Toolkit.graSetVisible( g.nativeData, isTgtVisible); */
                DVBNativeGraphics.setVisible( g, isTgtVisible );
                isVisible = isTgtVisible;
            }
            if ( isTgtVisible ) {
                if ( ( g.xClip == 0 ) && ( g.yClip == 0 ) &&
                    ( g.wClip == width ) && ( g.hClip == height ) ) {
                        DVBNativeGraphics.getClippedGraphics( g, tgt,
                            g.xOffset - xOffset, g.yOffset - yOffset, 0, 0,
                            tgt.width, tgt.height, false );
                        width = g.wClip;
                        height = g.hClip;
                } else {
                    DVBNativeGraphics.getClippedGraphics( g, tgt,
                        g.xOffset - xOffset, g.yOffset - yOffset, g.xClip,
                        g.yClip, g.wClip, g.hClip, false );
                }
            }
            xOffset = g.xOffset;
            yOffset = g.yOffset;
