...
/**
 * Window - Copyright (c) 1998 Transvirtual Technologies, Inc.  All rights
 * reserved. See the file "license.terms" for information on usage and
 * redistribution of this file.
 * @author P.C.Mehlitz
 */
public class Window extends Container {
    static Category CAT = Category.getInstance( Window.class );
    ...
    public void addNotify() {
        if ( nativeData != 0 ) // don't do it twice
            {
                return;
        }
	// if we have an owner that has not been created yet, do it now
        if ( ( owner != null ) && ( owner.nativeData == 0 ) ) {
            owner.addNotify();
        }
	...
	// (i.e. need a native parent before they can be addNotified by themselves)
         ...
         * // since setSize() triggers a layout, avaoid to do it before we
         * know the right size. if ( (width == 0) || (height == 0) ) {
         * // Bad! We have to defer setting a native size up to this point, because
         * // getPreferredSize() might use a careless LayoutManager who
         * doesn't check for // null peers. It really would be nice if we
         * would already have the right coords
         * // when creating the native window setSize( getPreferredSize()); }
         */
    }
    ...
    void createNative() {
        ...
        if ( bgClr instanceof DVBColor ) {
            bgAlpha = ( ( DVBColor )bgClr ).alpha;
        }

        nativeData = Toolkit.wndCreateWindow( ( owner != null ) ?
                       owner.nativeData : 0, x, y, width, height, cursor.type,
                       bgClr.nativeValue, bgAlpha );
    }
    ...
    public Graphics getGraphics() {
		...
		// (hence we partly have to use insets, not completely relying on deco)
            return DVBNativeGraphics.getGraphics( this, nativeData,
                DVBNativeGraphics.TGT_TYPE_WINDOW, u, v, insets.left,
                insets.top, width - deco.width, height - ( insets.top + insets.bottom ),
	...
	// ONLY to "(getPeer() != null)" check if we already passed addNotify()
        return ( ( flags & IS_ADD_NOTIFIED ) != 0 ) ?
            Toolkit.windowPeer : null;
    }

    final public String getWarningString() {
        if ( System.getSecurityManager().checkTopLevelWindow( this ) ==
            true ) {
                return ( null );
        }
    ...
    void process( FocusEvent event ) {
		...
		// (popup1-lost -> owner-gained ->post forward , popup2-gained, forward-gained ->popup2-lost)
            if ( ( nChildren > 0 ) &&
                !Toolkit.eventQueue.hasPendingEvents( null,
                FocusEvt.FOCUS_GAINED ) ) {
                    c = ShortcutHandler.focusNext( this );
                    if ( ( c != null ) && ( c != this ) ) {
                        AWTEvent.sendEvent(
                            FocusEvt.getEvent( c, FocusEvt.FOCUS_GAINED, false ), true );
                    }
            }
        }
    }
    ...
    public void removeNotify() {
		...
		// up in a KaffeServer context)
            if ( ( AWTEvent.activeWindow == this ) &&
                ( AWTEvent.keyTgt != null ) ) {
                    AWTEvent.sendEvent(
                        FocusEvt.getEvent( AWTEvent.keyTgt,
		...
		// removeNotify jitter)
            ...
            if ( ( wndListener != null ) ||
                ( eventMask & AWTEvent.WINDOW_EVENT_MASK ) != 0 ) {
                    AWTEvent.sendEvent(
                        WindowEvt.getEvent( this, WindowEvent.WINDOW_CLOSED ), false );
            }
        }
    }
    ...
    public void requestFocus() {
        if ( ( nativeData != 0 ) && ( ( flags & IS_VISIBLE ) != 0 ) ) {
	...
	// this is never called by a native toplevel resize
        if ( ( xNew == x ) && ( yNew == y ) && ( wNew == width ) &&
            ( hNew == height ) ) {
                return;
        } // avoid flicker of redundant reshapes
	...
	// invalidation during ComponentEvt.getEvent() (hello again, SwingSet..)
        ...
        if ( nativeData != 0 ) {
            ...
            Toolkit.wndSetBounds( nativeData, xNew, yNew, wNew, hNew,
    ...
    public void show() {
	CAT.debug(" java.awt.Window.show() called.");

        if ( nativeData == 0 ) {
            CAT.debug( "calling addNotify" );
            addNotify();
            CAT.debug( "returned from addNotify" );
        }
		...
		// is shown, and JDK sends this after it got shown, so this is the place
            if ( ( flags & IS_OPENED ) == 0 ) {
                ...
                if ( ( wndListener != null ) ||
                    ( eventMask & AWTEvent.WINDOW_EVENT_MASK ) != 0 ) {
                        AWTEvent.sendEvent(
                            WindowEvt.getEvent( this, WindowEvent.WINDOW_OPENED ), false );
                }
            }
        }
        CAT.debug( "Window::show returning" );
    }

    public void toBack() {
        if ( nativeData != 0 ) {
            Toolkit.wndToBack( nativeData );
        }
    }

    public void toFront() {
        if ( nativeData != 0 ) {
            Toolkit.wndToFront( nativeData );
        }
    }
}
