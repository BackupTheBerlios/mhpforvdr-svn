...
/**
 * Dialog - Copyright (c) 1998 Transvirtual Technologies, Inc.  All rights
...
public class Dialog extends Frame {
	...
	// setDialogInsets() from the native layer)
        ...
        dialogInsets = new Insets( guess.top, guess.left, guess.bottom,
            guess.right );
        dialogDeco = new Rectangle( guess.left, guess.top, ( guess.left + guess.right ),
            ( guess.top + guess.bottom ) );
        Toolkit.wndSetDialogInsets( guess.top, guess.left, guess.bottom,
            guess.right );
    }
	...
	// (no need to create fresh objects since they are insets()-copied anyway <sigh>)
        ...
        nativeData = Toolkit.wndCreateDialog( owner.nativeData, title, u, v, w,
            h, cursor.type, bgClr.nativeValue, 255,
    ...
    public Dimension preferredSize() {
        ...
        d.width += ( Defaults.DialogInsets.left +
            Defaults.DialogInsets.right );
        d.height += ( Defaults.DialogInsets.top +
            Defaults.DialogInsets.bottom );
        return ( d );
    }
