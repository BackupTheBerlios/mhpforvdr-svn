 ...
 * $File: HIcon.java $
 * Last changed on $Date: 2001/05/18 15:42:41 UTC $
 *
 ...
 * <p>
 * Icon is a widget that provides basic graphic display.
 * This HVisible extends HStaticIcon but is focus traversible.
 * It has two states: NORMAL_STATE and FOCUS_STATE.
...
public class HIcon extends HStaticIcon
  ...
  /* Inherited for Component: x, y, width, height, font
     ...
     Image is stored with setGraphicContent from HVisible */
  ...
  /** Constructor with initial image and geometry. Handled by parent */
  public HIcon(Image image,
               int x, int y, int width, int height) {
    super(image, x, y, width, height);
    initialize();
    /* By default, the image for FOCUS_STATE is the same */
    setGraphicContent(image, HState.FOCUSED_STATE);
  }
  ...
  /** Default look for all HIcon widgets */
  private static HGraphicLook defaultLook
    = new HGraphicLook(HGraphicLook.ICON_DECORATION);
  
  /** Set a new default look for the HIcon class.
      Note that this is not inherited directly from HStaticIcon because
      this default should be local to HIcon, not to its parent.
...
"New look is not an HGraphicLook and would not render HIcon properly");
    }
  }
...
public void processHFocusEvent(org.havi.ui.event.HFocusEvent evt) {
  ...
  Hashtable navigationMap = new Hashtable();
  ...
  */
  ...
  /** Process key events (from KeyListener interface) in search of
      ...
      This is duplicated in HText and other components */
