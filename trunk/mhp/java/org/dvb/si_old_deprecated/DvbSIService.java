...
public class DvbSIService extends TrivialSIInformation implements SIService {
...
DvbSIService(SIDatabase db, int nid, int tid, int sid, String name, boolean freeToAir) {
   super(db);
   this.nid=nid;
   this.tid=tid;
   this.sid=sid;
...
public org.davic.net.dvb.DvbLocator getDvbLocator() {
   try {
      return new org.davic.net.dvb.DvbLocator(nid, tid, sid);
...
public byte getRunningStatus() {
   return SIRunningStatus.RUNNING;
}
...
short[] someDescriptorTags) {
   return new SIRequestEvent(appData, listener, db, false, retrieveMode, getDvbLocator(), someDescriptorTags);
}
...
appData, SIRetrievalListener listener, short[] someDescriptorTags) {
   return new SIRequestEvent(appData, listener, db, true, retrieveMode, getDvbLocator(), someDescriptorTags);
}
