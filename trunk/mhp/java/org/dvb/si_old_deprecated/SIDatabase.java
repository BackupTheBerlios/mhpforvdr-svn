...
protected SIDatabase(int nativeData) {
...
private native int numDatabases();
private native int databasePointer(int index);


/* package-internal API */
/*void appendRequest(SIRequest r) {
   taskThread.append(r);
}


class SITaskThread extends Thread {
   java.util.LinkedList list;
   SITaskThread() {
      list=new java.util.LinkedList();
   }
   
   synchronized void append(SIRequest task) {
      if (!running) {
         running=true;
         start();
      }
   ...
   public void run() {
      try {
         while (running) {
            SIRequest task;
            if ( (task=ExecuteNext()) == null)
               sleep(5); //5ms=5000ns => usleep(5000)
            else
               task.Execute();
         }
         running=false;
   ...
   private synchronized SIRequest ExecuteNext() {
      ...
      while (it.hasNext()) {
         SIRequest task;
         try {
            task=(SIRequest)it.next();
         } catch (java.util.NoSuchElementException _) { return null; }
         
         if (task.CanExecute()) {
            try { 
               it.remove();
            }  catch (UnsupportedOperationException e) { e.printStackTrace(); return null; }
               catch (IllegalStateException _) { return null; }
            return task;
         }
      }
      return null;
...
short[] someDescriptorTags) {
   return new SIRequestDummy(appData, listener, this);
}
...
org.davic.net.dvb.DvbLocator dvbLocator, short[] someDescriptorTags) {
   SIRequest req=new SIRequestService(appData, listener, this, retrieveMode, dvbLocator, someDescriptorTags);
   return req;
}
