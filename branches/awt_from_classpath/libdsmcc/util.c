...
//A simple reference-counting copy-on-write array,
//taken from C++ FAQ Lite
CharArray::CharArray() : data_(new Data()) {
}
...
CharArray::CharArray(const CharArray &f) : data_(f.data_) {
   data_->Lock();
   ++ data_->count_;
   data_->Unlock();
}
...
void CharArray::assign(unsigned char*data, unsigned int size) {
...
void CharArray::assign(RawData data, unsigned int size) {
    // This method might need to change things in *data_
    // Thus it first checks if this is the only pointer to *data_
    if (data_->count_ > 1) {
      Data* d = new Data(*data_);
    ...
    data_->Lock();
    data_->assign(data.buffer, size<=data.length ? size : data.length);
    data_->Unlock();
}

void CharArray::assign(unsigned int size) {
    // This method might need to change things in *data_
    // Thus it first checks if this is the only pointer to *data_
    if (data_->count_ > 1) {
      Data* d = new Data(*data_);
...
void CharArray::Data::assign(unsigned char *d, unsigned int s) {
   if (size)
      delete[] data;
   size=s;
...
SimpleCharArray::SimpleCharArray(int si) {
   size=si;
   data=new unsigned char[size];
}
   
SimpleCharArray::SimpleCharArray(const SimpleCharArray &source) {
   size=source.size;
   data=source.data;
}

void SimpleCharArray::deepCopy() {
   unsigned char *temp=new unsigned char[size];
   memcpy(temp, data, size);
   data=temp;
}

SimpleCharArray& SimpleCharArray::operator=(const SimpleCharArray &source) {
   size=source.size;
   data=source.data;
   return *this;
}

void SimpleCharArray::destroy() {
   delete[] data;
}
