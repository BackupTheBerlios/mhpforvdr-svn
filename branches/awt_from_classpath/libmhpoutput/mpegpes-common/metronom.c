...
#define xprintf_this__xine__XINE_VERBOSITY_DEBUG_(...) printf(...)
#define xprintf_this__xine__XINE_VERBOSITY_LOG_(...) printf(...)
#define xprintf_this__xine__XINE_VERBOSITY_NONE_(...) printf(...)
...
static scr_plugin_t* unixscr_init () {
  ...
  this = (unixscr_t *) calloc(sizeof(unixscr_t));
...
static void metronom_handle_discontinuity (metronom_t *this, int type,
  ...
  //TODO Replicate functionality of line above
  cur_time = 0;
  ...
  switch (type) {
    ...
    case DISC_STREAMSEEK:
      ...
      this->video_drift = 0;
      xprintf_this__xine__XINE_VERBOSITY_DEBUG_((
    ...
    case DISC_RELATIVE:
      if (this->video_vpts < cur_time) {
        ...
        if (this->audio_vpts > cur_time) {
          ...
          xprintf_this__xine__XINE_VERBOSITY_DEBUG_((( "video vpts adjusted to audio vpts\n");
        } else {
          ...
          this->force_audio_jump = 1;
          this->video_drift = 0;
          xprintf_this__xine__XINE_VERBOSITY_DEBUG_((( "vpts adjusted with prebuffer to %" PRId64 "\n",
	    this->video_vpts);
        }
      } else {
        /* video */
        if (this->audio_vpts < cur_time) {
          /* video, no sound */
          this->audio_vpts = this->video_vpts;
          xprintf_this__xine__XINE_VERBOSITY_DEBUG_((( "audio vpts adjusted to audio vpts\n");
        } else {
          /* video + audio */
        }
      }
...
static void metronom_handle_video_discontinuity (metronom_t *this, int type,
  ...
  xprintf_this__xine__XINE_VERBOSITY_DEBUG_((( "video discontinuity #%d, type is %d, disc_off %" PRId64 "\n",
  ...
  if (this->have_audio) {
    while (this->audio_discontinuity_count <
      ...
      xprintf_this__xine__XINE_VERBOSITY_DEBUG_((( "waiting for audio discontinuity #%d\n", 
        this->video_discontinuity_count);
...
static void metronom_got_video_frame (metronom_t *this, vo_frame_t *img) {
  ...
  if (pts && pts != this->last_video_pts) {
    ...
    if ((abs (diff) > VIDEO_DRIFT_TOLERANCE) || (this->force_video_jump)) {
      ...
      xprintf_this__xine__XINE_VERBOSITY_DEBUG_((( "video jump\n");
...
static void metronom_handle_audio_discontinuity (metronom_t *this, int type,
  ...
  xprintf_this__xine__XINE_VERBOSITY_DEBUG_((( "audio discontinuity #%d, type is %d, disc_off %" PRId64 "\n",
  ...
  if (this->have_video) {
    ...
    while ( this->audio_discontinuity_count >
      ...
      xprintf_this__xine__XINE_VERBOSITY_DEBUG_((( "waiting for in_discontinuity update #%d\n", 
	      this->audio_discontinuity_count);
...
static int64_t metronom_got_audio_samples (metronom_t *this, int64_t pts, 
  ...
  if (pts && pts != this->last_audio_pts) {
    ...
    if((abs(diff) > AUDIO_DRIFT_TOLERANCE) || (this->force_audio_jump)) {
      ...
      this->audio_drift_step = 0;
      xprintf_this__xine__XINE_VERBOSITY_DEBUG_((( "audio jump, diff=%" PRId64 "\n", diff);
    } else {
...
static void metronom_set_option (metronom_t *this, int option, int64_t value) {
  ...
  case METRONOM_AV_OFFSET:
    ...
    xprintf_this__xine__XINE_VERBOSITY_LOG_( "av_offset=%" PRId64 " pts\n", this->av_offset);
    break;
  case METRONOM_SPU_OFFSET:
    this->spu_offset = value;
    xprintf_this__xine__XINE_VERBOSITY_LOG_( "spu_offset=%" PRId64 " pts\n", this->spu_offset);
    break;
  case METRONOM_ADJ_VPTS_OFFSET:
     ...
     */
    xprintf_this__xine__XINE_VERBOSITY_LOG_( "fixing sound card drift by %" PRId64 " pts\n", value);
    break;
  case METRONOM_PREBUFFER:
    this->prebuffer = value;
    xprintf_this__xine__XINE_VERBOSITY_LOG_( "prebuffer=%" PRId64 " pts\n", this->prebuffer);
    break;
  default:
    xprintf_this__xine__XINE_VERBOSITY_NONE_( "unknown option in set_option: %d\n", option);
  }
...
static int64_t metronom_get_option (metronom_t *this, int option) {
  ...
  xprintf_this__xine__XINE_VERBOSITY_NONE_( "unknown option in get_option: %d\n", option);
  return 0;
}
...
static scr_plugin_t* get_master_scr(metronom_clock_t *this) {
  ...
  if (select < 0) {
    xprintf_this__xine__XINE_VERBOSITY_NONE_( "panic - no scr provider found!\n");
    return NULL;
  }
...
metronom_t * _x_metronom_init (int have_video, int have_audio/*, xine_t *xine*/) {
  ...
  metronom_t *this = malloc (sizeof (metronom_t));
...
metronom_clock_t *_x_metronom_clock_init(/*xine_t *xine*/)
{
  ...
  if ((err = pthread_create(&this->sync_thread, NULL,
    ...
    xprintf_this__xine__XINE_VERBOSITY_NONE_( "cannot create sync thread (%s)\n",
	    strerror(err));
