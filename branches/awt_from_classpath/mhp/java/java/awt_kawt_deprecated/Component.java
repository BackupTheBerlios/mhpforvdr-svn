...
import org.dvb.ui.DVBNativeGraphics;
 ...
 * @author P.C.Mehlitz
 */
abstract public class Component extends Object implements ImageObserver,
    MenuContainer, Serializable {
        static final Category cat = Category.getInstance( Component.class );
        ...
        Locale locale;
        PopupMenu popup;
        public Rectangle deco = noDeco;
        public int flags = IS_VISIBLE;

        /**
         * linkedGraphs is a list of WeakReferences to DVBNativeGraphics
        ...
        public final static int IS_NATIVE_LIKE     = 0x100;
        final static int IS_OPENED                 = 0x200;
        public final static int IS_ADD_NOTIFIED    = 0x400;
        ...
        public final static int IS_DIRTY           = 0x8000;
        final static int IS_MOUSE_AWARE            = 0x10000;
        final static int IS_TEMP_HIDDEN            = 0x20000;
        final static int IS_SHOWING =
            IS_ADD_NOTIFIED | IS_PARENT_SHOWING | IS_VISIBLE;
	 ...
	 */
        ...
        public void addFocusListener( FocusListener newListener ) {
            focusListener = AWTEventMulticaster.add( focusListener,
                newListener );
        }
        ...
        public void addMouseMotionListener( MouseMotionListener newListener ) {
            motionListener = AWTEventMulticaster.add( motionListener,
                newListener );
            flags |= IS_MOUSE_AWARE;
        }
        ...
        public void addNotify() {
			...
			// unless we turn BarMenus into isNativeLike, which is bad)
                    if ( ( ( ( parent.flags & IS_OLD_EVENT ) != 0 ) ||
                        props.useOldEvents ) ) {
	...
	// on its constness (e.g. for InternalFrame dragging)
        ...
        void checkMouseAware() {
            if ( ( ( eventMask & AWTEvent.DISABLED_MASK ) == 0 ) &&
                ( ( mouseListener != null ) || ( motionListener != null ) ||
    ...
    // undocumented method)
        ...
        public Color getBackground() {
            ...
            return null; //return Color.white;
        }
    ...
    // (they had no protected ctor in 1.0.2)
        ...
        public Graphics getGraphics() {
            if ( ( flags & IS_ADD_NOTIFIED ) != 0 ) {
                return DVBNativeGraphics.getClippedGraphics( null, this, 0, 0,
    ...
    // this has to be resolved for Applets
        ...
        int getNativeData() {
            return 0; // no nativeData, all lightweight
        }
    ...
    // ONLY to "(getPeer() != null)" check if we already passed addNotify()
            return ( ( flags & IS_ADD_NOTIFIED ) != 0 ) ?
                Toolkit.lightweightPeer : null;
        }
        ...
        /**
         * @deprecated, use getSize() this is never called automatically,
         * override getSize in derived classes to change the default behavior
         */
        ...
        Component getToplevel() {
            Component c;
            for ( c = this; !( c instanceof Window ) && c != null;
                c = c.parent );
            return c;
        }
    ...
    // in the JDK javadoc documentation.
        ...
        public boolean inside( int x, int y ) {
            ...
            /**
             ** // If we want to deal with components being bigger than their
             * parents, // we have to check for parent.contains(), too.
             * However, this is not // done by the JDK, and we therefor skip
             * it for now x += this.x; y += this.y;
             * for ( Container c=parent; c!= null; x += c.x, y += c.y, c =
             * c.parent ) { if ( (x < 0) || (y < 0) || (x > c.width) || (y >
             * c.height) ) return false; }
             */
            return true;
        }
	...
	// compare the costs of this with the standard upward iteration
        ...
        public synchronized void linkGraphics( DVBNativeGraphics g ) {
	...
	// nothing to do here, that all has to be donw in subclasses
        ...
        /* void paintBorder () {  modified on 5Mar2001 by joe@convergence.de*/
        ...
        /* void paintBorder ( Graphics g ) { modified on 5Mar2001 by
        joe@convergence.de*/
        void kaffePaintBorder( Graphics g ) {
            kaffePaintBorder( g, 0, 0, 0, 0 );
        }

        /* void paintBorder ( Graphics g, int left, int top, int right, int
        bottom ) { modified on 5Mar2001 by joe@convergence.de*/
        void kaffePaintBorder( Graphics g, int left, int top, int right,
            int bottom ) {
	...
	// way JDK obviously does it (just directly calling getMinimumSize)
        ...
        public boolean prepareImage
            ( Image image, int width, int height, ImageObserver obs ) {
	...
	// we don't know nothing about ItemEventListeners
        ...
        //X cat.debug( "component " + toString() + " got " + e );
            ...
            if ( ( keyListener != null ) ||
                ...
                //X cat.debug("going to process it");
                    processEvent( e );
            }
            if ( ( flags & IS_OLD_EVENT ) != 0 ) {
                postEvent( Event.getEvent( e ) );
	...
	// with listeners)
		...
		// if ( (eventMask & AWTEvent.DISABLED_MASK) == 0 )
                //X cat.debug("calling processKeyEvent");
        ...
        protected void processKeyEvent( KeyEvent event ) {
            UserEvent ue=new UserEvent(event.getSource(), UserEvent.UEF_KEY_EVENT, 
                                       event.id, event.getKeyCode(), event.getModifiers());
            if (EventManager.getInstance().DispatchEvent(ue)) {
               event.consume();
               return; //exclusive event, no longer dispatched
            }
            if ( keyListener != null ) {
                switch ( event.id ) {
        ...
        void processPaintEvent( int id, int ux, int uy, int uw, int uh ) {
            DVBNativeGraphics g =
                DVBNativeGraphics.getClippedGraphics( null, this, 0, 0, ux, uy,
                uw, uh, false );
            if ( g != null ) {
                g.enterBuffered();
	...
	// but we might have resident Graphics which have to be notified
        ...
        public void removeFocusListener( FocusListener listener ) {
            focusListener = AWTEventMulticaster.remove(
        ...
        public void removeMouseListener( MouseListener listener ) {
            mouseListener = AWTEventMulticaster.remove(
                mouseListener, listener );
            checkMouseAware();
        }

        public void removeMouseMotionListener( MouseMotionListener listener ) {
            motionListener = AWTEventMulticaster.remove(
	...
	// (note that we don't have to care for native events because of unregisterSource())
        ...
        public void repaint( long ms, int x, int y, int width, int height ) {
            if ( ( flags & IS_SHOWING ) == IS_SHOWING ) {
	//X cat.debug("repainting (" + x + ", " + y + " - " + width + "x" + height + ", IS_SHOWING set)");
        // be paranoid, some clients might request repaints outside their own turf
                ...
                if ( parent != null ) {
                    parent.repaint( ms, this.x + x, this.y + y,
                        width, height );
                } else {
            //processPaintEvent( PaintEvent.UPDATE, x, y, width, height);
                    Toolkit.eventQueue.postPaintEvent( PaintEvent.UPDATE, this,
                        x, y, width, height );
                }
            }
        }
	...
	// which have not even been addNotified yet (hence no parent)
		...
		// by a subsequent requestFocus of the toplevel
                ...
                if ( topNew != AWTEvent.activeWindow ) { // this involves a change of active toplevels
                    ...
                    Toolkit.eventQueue.postFocusEvent(
                        FocusEvt.getEvent( this,
         ...
         * this is never called automatically, override setBounds in derived
	...
	// for compatibility reasons (Swing etc.)
            ...
            int id = sized ? ComponentEvent.COMPONENT_RESIZED :
	...
	// Don't do anything if we don't change anything.
			...
			// explicitly moved the mouseTgt or one of its parents (maybe in a mouse modal drag!)
                    if ( MouseEvt.mouseDragged ) {
                        for ( Component c = AWTEvent.mouseTgt; c != null;
                            c = c.parent ) {
                                if ( c == this ) {
                                    MouseEvt.moveMouseTgt( ( xNew - x ),
                                        ( yNew - y ) );
                                    break;
                                }
                        }
                    ...
                    if ( ( flags & IS_SHOWING ) == IS_SHOWING ) {
                        ...
                        if ( ( cmpListener != null ) ||
                            ( eventMask &
                            AWTEvent.COMPONENT_EVENT_MASK ) != 0 ) {
                                Toolkit.eventQueue.postEvent(
                                    ComponentEvt.getEvent( this, id ) );
                        }
                        propagateReshape();
        ...
        /**
         * @deprecated, use setSize( d) this is never called automatically,
         * override setSize in derived classes to change the default behavior
         */
	...
	// might break compatibility of derived app classes. The workhorse is still reshape
         ...
         * this is never called automatically, override setSize in derived
         * classes to change the default behavior
         */
	...
	// rely in this unspec. behavior - we have to repaint automatically for native-likes
            if ( ( flags & ( IS_PARENT_SHOWING | IS_VISIBLE | IS_ADD_NOTIFIED |
                IS_NATIVE_LIKE ) ) ==
                ( IS_PARENT_SHOWING | IS_VISIBLE | IS_ADD_NOTIFIED ) ) {
                // Enno: i removed this repaint (same for setForeground) because it could
                // introduce inifite loops when this method was called from within paint()
                // I think it's ok, because it's it's not specified that a setBackground()
                // should trigger a repaint (and the above comment states that it's a fix
                // for legacy apps anyway.)
		//repaint();
            }
        }
	...
	// see setBackground for further details about why to repaint just visible native-likes
            if ( ( flags & ( IS_PARENT_SHOWING | IS_VISIBLE | IS_ADD_NOTIFIED |
                IS_NATIVE_LIKE ) ) ==
                ( IS_PARENT_SHOWING | IS_VISIBLE | IS_ADD_NOTIFIED ) ) {
                // see setBackground() if you want to know why the following
	...
	// might break compatibility of derived app classes. The workhorse is still reshape
        ...
        public void setVisible( boolean b ) {
	     cat.debug("This instance ("+this+") is a " + this.getClass().getName()); 
             show( b );
        }

        public void show() {
        cat.debug("show called");
	// DEP this should be in setVisible !! But we have to keep it here
	// for compatibility reasons (Swing etc.)
            if ( ( flags & IS_VISIBLE ) == 0 ) {
                flags |= IS_VISIBLE;
                flags &= ~IS_TEMP_HIDDEN;
		//X cat.debug("making visible, parent: " + parent);
	  // if we are a toplevel, the native window manager will take care
	  // of repainting
                if ( ( parent != null ) &&
                    ( ( parent.flags & IS_LAYOUTING ) == 0 ) ) {
			  ...
			  //parent.repaint( x, y, width, height);
			    //X cat.debug("will repaint");
                                repaint();
                        }
                        if ( ( parent.flags & IS_VALID ) != 0 ) {
			    //X cat.debug("will invalidate");
                            parent.invalidate();
                        }
                } else {
		    //X cat.debug("repainting without a parent");
		    // fixed by enno, cs 15.8.2001
		    //		    repaint();
                }
                if ( ( cmpListener != null ) ||
                    ( eventMask & AWTEvent.COMPONENT_EVENT_MASK ) != 0 ) {
		    //X cat.debug("posting componentshown event");
                        Toolkit.eventQueue.postEvent(
                            ComponentEvt.getEvent( this,
		...
		// update any resident graphics objects		
                if ( linkedGraphs != null ) {
		    //X cat.debug("updating linked graphics objs");
                    updateLinkedGraphics();
                }
            }
            cat.debug("exiting show");
        }
	...
	// We have to return fresh objects because of the same reasons like in bounds()
        ...
        public synchronized void unlinkGraphics( DVBNativeGraphics g ) {
            GraphicsLink li, last, next;
            Object lg;
	// do some cleanup as we go
            for ( li = linkedGraphs, last = null; li != null; ) {
