...
public class PaintEvt extends PaintEvent {
    ...
    PaintEvt( Component src, int evtId, long ms, int x, int y, int width,
        int height ) {
    ...
    protected void dispatch() {
        if ( source != null ) {
            ( ( Component )source ).processPaintEvent( id, x, y,
                width, height );
        } else {
            System.err.println( "java.awt.PaintEvt: source == null!" );
            Thread.dumpStack();
        }
        recycle();
    }

    static synchronized PaintEvt getEvent
        ( Component source, int id, long ms, int x, int y, int width,
        int height ) {
            if ( cache == null ) {
    ...
    static synchronized PaintEvt getEvent
        ( int srcIdx, int id, int x, int y, int width, int height ) {
            PaintEvt e;
    ...
    protected boolean isObsoletePaint( Object src, int x, int y,
        int w, int h ) {
            return ( ( src == source ) && ( x <= this.x ) && ( y <= this.y ) &&
