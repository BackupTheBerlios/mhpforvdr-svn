...
public class KeyEvt extends KeyEvent {
    static final Category CAT = Category.getInstance( KeyEvt.class );
    static KeyEvt cache;

    public KeyEvt( Component src, int evtId, long time, int mods, int kCode,
        char kChar ) {
    ...
    protected void dispatch() {
        ...
        if ( id == KEY_PRESSED ) {
			...
			// update modifiers for function keys
                switch ( keyCode ) {
                    case VK_SHIFT:
                        inputModifier |= SHIFT_MASK;
                    break;
                    case VK_CONTROL:
                        inputModifier |= CTRL_MASK;
                    break;
                    case VK_ALT:
                        inputModifier |= ALT_MASK;
                    break;
                    case VK_META:
                        inputModifier |= META_MASK;
                }
            }
		// we interpret the setEnabled spec in a way that we don't emit events for disabled comps
            if ( ( AWTEvent.keyTgt.eventMask &
                AWTEvent.DISABLED_MASK ) == 0 ) {
                    AWTEvent.keyTgt.process( this );
			//X CAT.debug("Event processed: consumed? " + consumed );

                    /* ENNO: kaffe prevents KEY_TYPED-Events if
                    KEY_PRESSED-Events are consumed. this might seem somewhat
                    reasonable but no other JDK does this. you
                    cannot prevent KEY_TYPED events. */

                    /* if ( !consumed && !ShortcutHandler.handle( this) ) { */
                    if ( !ShortcutHandler.handle( this ) ) {
    ...
    static synchronized KeyEvt getEvent
        ( Component source, int id, int keyCode, int keyChar, int modifier ) {
            KeyEvt e;
            long when = System.currentTimeMillis();
            if ( cache == null ) {
                e = new KeyEvt( source, id, when, modifier, keyCode,
                    ( char )keyChar );
            } else {
                e = cache;
                cache = ( KeyEvt )e.next;
    ...
    static synchronized KeyEvt getEvent
        ( int srcIdx, int id, int keyCode, int keyChar, int modifier ) {
            KeyEvt e;
	...
	// (e.g. fvwm2 during an initial window positioning)
            ...
            if ( cache == null ) {
                e = new KeyEvt( source, id, when, inputModifier, keyCode,
                    ( char )keyChar );
            } else {
                e = cache;
                cache = ( KeyEvt )e.next;
