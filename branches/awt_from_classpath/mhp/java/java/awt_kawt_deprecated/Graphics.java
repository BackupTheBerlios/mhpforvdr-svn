...
abstract public class Graphics {
    static Category CAT = Category.getInstance( Graphics.class );
    ...
    abstract public boolean drawImage
    ...
    abstract public boolean drawImage
        ( Image img, int x, int y, int width, int height, Color background,
    ...
    abstract public boolean drawImage
        ( Image img, int dx0, int dy0, int dx1, int dy1, int sx0, int sy0,
        int sx1, int sy1, Color bgColor, ImageObserver observer );
    ...
    abstract public void drawPolyline
        ( int xPoints[], int yPoints[], int nPoints );
    ...
    abstract public void drawRoundRect
    ...
    abstract public void fillPolygon
        ( int xPoints[], int yPoints[], int nPoints );

    abstract public void fillRect( int x, int y, int width, int height );

    abstract public void fillRoundRect
        ( int x, int y, int width, int height, int arcWidth, int arcHeight );
    ...
    public void paintChild( Component c, boolean isUpdate ) {
	...
	// a nasty app problem with Canvases and Panels, see DVBNativeGraphics

        /*	if ( (c.flags & Component.IS_ASYNC_UPDATED) != 0 ){
        c.repaint(); return; }*/

        /*	Graphics g = create( c.x, c.y, c.width, c.height);
        if ( g != null ) { if ( isUpdate ) c.update( g); else c.paint( g);
        g.dispose(); }*/
        setBackColor( c.bgClr );
        translate( c.x, c.y );
        clipRect( 0, 0, c.width, c.height );
        if ( isUpdate ) {
            c.update( this );
        } else {
            c.paint( this );
    ...
    public String toString() {
        return ( getClass().getName() + " [" + getFont() + "," +
            getColor() + ']' );
    }
