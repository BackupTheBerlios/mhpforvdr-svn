...
public class Image {
    ...
    final static int MASK = ImageObserver.WIDTH | ImageObserver.HEIGHT |
        ...
        ImageObserver.FRAMEBITS | ImageObserver.ALLBITS | ImageObserver.ERROR |
        ImageObserver.ABORT;
    final static int PRODUCING = 1 << 8;
    final public static int READY =
        ImageObserver.WIDTH | ImageObserver.HEIGHT |
        ImageObserver.ALLBITS | PRODUCING;
    final public static int SCREEN               = 1 << 10;
    public final static int BLOCK_FRAMELOADER    = 1 << 11;
    public final static int IS_ANIMATION         = 1 << 12;
    ...
    // so everything != 0 causes async production
        if ( ( Defaults.MemImageSrcThreshold == 0 ) &&
            ( producer instanceof MemoryImageSource ) ) {
                flags |= PRODUCING;
  ...
  // -------------------------------------------------------------------------
    ...
    synchronized int checkImage
        ( int w, int h, ImageObserver obs, boolean load ) {
    ...
    // we can't produce on-screen images
        if ( nativeData != 0 && ( flags & ImageObserver.ALLBITS ) != 0 ) {
    ...
    public Graphics getGraphics() {
        if ( ( flags & SCREEN ) == 0 || nativeData == 0 ) {
            return null;
        } else {
            return DVBNativeGraphics.getGraphics( this, nativeData,
                DVBNativeGraphics.TGT_TYPE_IMAGE, 0, 0, 0, 0, width, height,
    ...
    public Object getProperty( String name, ImageObserver observer ) {
        if ( props == null ) {
            if ( ( flags & ( ImageObserver.FRAMEBITS |
    ...
    static Image getUnknownImage() {
        if ( unknownImage == null ) {
            ...
            if ( g != null ) {
                ...
                g.draw3DRect( 0, 0, d - 1, d - 1, false );
                g.dispose();
            }
        }
        return unknownImage;
    ...
    public static boolean loadImage
        ( Image img, int w, int h, ImageObserver obs ) {
	...
	// there's work ahead, kick it off
                        ...
                        /* * We now load the image data synchronously - it's
                        not clear * that's what you should actually do but it
                        fixes a problem * with the Citrix ICA client.
                        (Tim 1/18/99) */
    ...
    private boolean scale( int w, int h ) {
        if ( ( flags & SCREEN ) != 0 || ( flags & ImageObserver.ALLBITS ) == 0 ||
            nativeData == 0 ) {
                return ( false );
        }
        if ( w != width || h != height ) {
            int oldNativeData = nativeData;
    ...
    // Addison-Wesley documentation, but is what is says in the JDK javadoc documentation.	
      ...
      // to notify *all* elements regardless of any removals
            for ( Enumeration e = VectorSnapshot.getCached( ( Vector )observers );
                e.hasMoreElements(); ) {
    ...
    public String toString() {
        String s = getClass().getName() + " [" + width + ',' + height +
            ( ( nativeData != 0 ) ? ", native" : ", non-native" ) +
...
class ImageFrameLoader extends Thread {
    Image img;
    ...
    // got the first frame propperly reported, i.e. we start with flipping to the next one
      ...
      // wait until we get requested the next time (to prevent being spinned around by a MediaTracker)
            ...
            if ( dt < latency ) {
                try {
                    Thread.sleep( latency - dt );
                } catch ( Exception _ ) {
                }
            }
            if ( ( latency == 0 ) || ( dt < 2 * latency ) ) {
                if ( ( ptr = Toolkit.imgGetNextFrame( img.nativeData ) )
                    == 0 ) {
                        break;
                }
            } else {
	...
	// animation with delta frames (and the first one sets the background)
            ...
            if ( (img.width != w) || (img.height != h) ){ img.width = w;
            img.height = h; img.stateChange(
            ImageObserver.WIDTH|ImageObserver.HEIGHT, 0, 0, img.width,
            img.height); } */
            img.flags |= Image.BLOCK_FRAMELOADER;
            img.stateChange( ImageObserver.FRAMEBITS, 0, 0, img.width,
                img.height );
...
class ImageLoader implements ImageConsumer, Runnable {
    Image queueHead;
    Image queueTail;
    Image img;
    ...
    public synchronized void imageComplete( int status ) {
      ...
      // the whole external image has been read in already (no IO required, anymore)
            ...
            if ( ( status & IMAGEERROR ) != 0 ) {
                s |= ImageObserver.ERROR;
            }
            if ( ( status & IMAGEABORTED ) != 0 ) {
    ...
    // the producer might decide to stop if it doesn't have another consumer
        ...
        if ( this == asyncLoader ) {
            notify();
        } // in case we had an async producer
    }
    ...
    static void loadSync( Image img ) {
        synchronized( syncLoader ) {
            System.out.println( "ImageLoader.loadSync: syncLoader: " +
                syncLoader + ", " + " img: " + img );
            if ( img != null ) {
                System.out.println( "ImageLoader.loadSync: img.producer: " +
                    img.producer );
                System.out.println( "ImageLoader.loadSync: img.nativeData: " +
                    img.nativeData );
            }
            syncLoader.img = img;
            if ( img.producer != null ) {
    ...
    // target for subsequent setPixel() calls
        ...
        img.stateChange( ImageObserver.WIDTH | ImageObserver.HEIGHT, 0, 0,
            width, height );
    }
    ...
    public void setPixels( int x, int y, int w, int h, ColorModel model,
      ...
      // producer trouble, we haven't got dimensions yet
            ...
            if ( model instanceof IndexColorModel ) {
                ...
                System.err.println( "Unhandled colorModel: " +
                    model.getClass() );
            }
    }
    ...
    public void setPixels( int x, int y, int w, int h, ColorModel model,
      ...
      // in case our pels aren't default RGB yet, convert them using the ColorModel
                ...
                System.out.println( "Unhandled colorModel: " +
                    model.getClass() );
            }
            Toolkit.imgSetRGBPels( img.nativeData, x, y, w, h, pixels,
                offset, scansize );
            img.stateChange( ImageObserver.SOMEBITS, x, y, w, h );
    }
    ...
    public void setProperties( Hashtable properties ) {
        ...
        img.stateChange( img.flags | ImageObserver.PROPERTIES, 0, 0, img.width,
            img.height );
    }
}
 ...
 */
class ImageNativeProducer implements ImageProducer {
    Object consumer;
    Object src;
    int off;
    int len;
    ...
    void produceFrom( File file ) {
        int ptr;
        if ( file.exists() && ( ptr =
            Toolkit.imgCreateFromFile( file.getAbsolutePath() ) ) != 0 ) {
                produceFrom( ptr );
        } else {
        ...
        try {
            ...
            if ( conn != null ) {
                ...
                if ( in != null ) {
                    out.readFrom( in );
                    in.close();
                    int ptr = Toolkit.imgCreateFromData( out.getBuffer(),
                        0, out.size() );
                    if ( ptr != 0 ) {
                        produceFrom( ptr );
                        return;
    ...
    void produceFrom( byte[] data, int off, int len ) {
        int ptr;
        if ( ( ptr = Toolkit.imgCreateFromData( data, off, len ) ) != 0 ) {
            produceFrom( ptr );
        } else {
            imageComplete( ImageConsumer.IMAGEERROR |
                ImageConsumer.IMAGEABORTED );
    ...
    void setDimensions( int width, int height ) {
        if ( consumer instanceof ImageConsumer ) {
            ...
            int i, n = v.size();
            for ( i = 0; i < n; i++ ) {
                ( ( ImageConsumer )v.elementAt( i ) ).setDimensions(
                    width, height );
            }
        }
    }
    ...
    void setPixels( int x, int y, int w, int h, ColorModel model, int pixels[],
        int offset, int scansize ) {
            if ( consumer instanceof ImageConsumer ) {
                ( ( ImageConsumer )consumer ).setPixels( x, y, w, h, model,
                    pixels, offset, scansize );
            } else if ( consumer instanceof Vector ) {
                Vector v = ( Vector )consumer;
                int i, n = v.size();
                for ( i = 0; i < n; i++ ) {
                    ( ( ImageConsumer )v.elementAt( i ) ).setPixels( x, y, w,
                        h, model, pixels, offset, scansize );
                }
            }
    }
    ...
    public void startProduction( ImageConsumer ic ) {
        ...
        if ( src instanceof File ) {
            ...
            System.err.println( "unsupported production source: " +
                src.getClass() );
        }
        removeConsumer( ic );
    }
