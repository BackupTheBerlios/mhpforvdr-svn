...
public Graphics getGraphics () {
   if ((flags & SCREEN) == 0 || nativeData == 0) {
      ...
      return MHPScreen.getImageGraphics( this );
   }
}

long getNativeSurface() {
   return Toolkit.imgGetSurface(nativeData);
}
...
static Image getUnknownImage (){
   if ( unknownImage == null ){
      int d = MHPScreen.getResolutionX() / 64;
...
//not official API
static Image getEmptyImage(int x, int y) {
...
private boolean scale (int w, int h) {
   if ((flags & SCREEN) != 0 || (flags & ImageObserver.ALLBITS) == 0 || nativeData == 0) {
      return (false);
   }
   if (w != width || h != height) {
...
//DirectFB's buffer directly. Currently leave it as it is.
    ...
    public int getRGB( int x, int y ) {
        if ((flags & SCREEN) != 0 && x<width && y<height && x>=0 && y >=0)
            return Toolkit.imgGetRGB(nativeData, x, y);
        return 0;
    }
    ...
    public int[] getRGB( int startX, int startY, int w, int h, int[] rgbArray, int offset, int scansize ) {
        if ((flags & SCREEN) == 0 || startX+w >= width || startY+h >= height || w<0 || h<0)
            return null;
        ...
        Toolkit.imgGetRGBRegion(nativeData, startX, startY, w, h, rgbArray, offset, scansize);
        return rgbArray;
    }
    ...
    public synchronized void setRGB( int x, int y, int rgb ) {
         if ((flags & SCREEN) != 0 && x<width && y<height && x>=0 && y >=0)
             Toolkit.imgSetRGB(nativeData, x, y, rgb);
    }
    ...
    public void setRGB( int startX, int startY, int w, int h, int[] rgbArray, int offset, int scansize ) {
        if ((flags & SCREEN) == 0 || startX+w >= width || startY+h >= height || w<0 || h<0)
            return;
        Toolkit.imgSetRGBRegion(nativeData, startX, startY, w, h, rgbArray, offset, scansize);        
    }
    ...
    public DVBBufferedImage getSubimageDVB( int x, int y, int w, int h ) 
        ...
        ret.nativeData = Toolkit.imgGetSubImage( nativeData, x, y, w, h );
        ret.width = w;
        ret.height = h;
        ret.flags = READY | SCREEN;
        return (DVBBufferedImage)ret;
    }
