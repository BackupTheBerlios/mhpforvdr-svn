...
synchronized void postPaintEvent ( int id, Component c, int x, int y, int width, int height ) {
   AWTEvent e = localQueue;
   //System.out.println("EventQueue.postPaintEvent for "+c);
   ...
   if ( e != null ) {
      do {
         if ( (e.id == id) && ((PaintEvt)e).solicitRepaint( c, x, y, width, height) ){
            //System.out.println("EventQueue: returning because of solicitRepaint");
            return;
         }
         if ( e.next == null ) {
            break;
      ...
      //System.out.println("EventQueue: e.next = getEvent");
      e.next = localEnd = PaintEvt.getEvent( c, id, 0, x, y, width, height);
   }
   else {
      //System.out.println("EventQueue: localQueue = getEvent");
