...
IDirectFBSurface::IDirectFBSurface (IDirectFBSurface_C *idirectfbsurface)
{
     refs  = 1;
     iface = idirectfbsurface;
}
...
void IDirectFBSurface::BatchBlit (IDirectFBSurface         *source,
                                  ...
                                  int                       num)
{
     DFBCHECK( iface->BatchBlit (iface, source->get_iface(), source_rects, dest_points, num) );
}
...
{
     DFBCHECK( iface->FillSpans (iface, y, spans, num_spans) );
}
